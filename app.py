...
# --------------------------------------------------------------------------
# UI: Hero header — shared by BOTH pages. Contains ONLY branding
# (logo, title, tagline, feature pills, description). No page-specific
# content lives here, so it never leaks the wrong heading onto the
# wrong page.
# --------------------------------------------------------------------------
...
def render_upload_page(base_url: str) -> None:
    ...
    # Page-specific heading lives HERE, not in the shared header, so it
    # only ever shows up on the upload page. label_visibility="collapsed"
    # on the uploader stops Streamlit from rendering its own default
    # "Choose a file to share" label underneath this one.
    st.markdown('<div class="ss-section-title">📤 Choose a file to share</div>', unsafe_allow_html=True)
